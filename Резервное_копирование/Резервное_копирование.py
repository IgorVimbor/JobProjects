...
from modul_copyfile import Copy_file


file_base = '//Server/otk/Support_files_не_удалять!!!/Резервное копирование_перечень файлов.txt'
...
class ListFrame(tk.Frame):
    def __init__(self, master):
        ...
        self.scroll = tk.Scrollbar(
            self, orient=tk.VERTICAL, command=self.list.yview)
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.geometry('%dx%d+%d+%d' % (width, heigh,
        ...
        self.progress_bar = ttk.Progressbar(
            self, orient="horizontal", mode="determinate", length=400, maximum=100, value=0)
    ...
    def start_copy(self):
        ...
        if flag:  # если копирование завершено без ошибок и copy_file() вернула True
            ...
            if messagebox.askokcancel('СООБЩЕНИЕ', 'Все файлы скопированы.\nЗакрыть программу?'):
                self.destroy()
