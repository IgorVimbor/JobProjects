...
def get_itog():   # функция подготовки выборки из базы и вывода на экран
    ...
    path = '//Server/otk/1 ГАРАНТИЯ на сервере/' + str(date.today().year) + '-2019_ЖУРНАЛ УЧЁТА.xls'  # путь до файла с учетом года
    wb = xlrd.open_workbook(path)  # открываем файл (вставляем свой путь до файла с базой)
    sheet = wb.sheet_by_name(god)  # создаем workbook для работы по имени Листа с соответствующим годом

    # список двигателей и актов рекламаций из всей базы
    cells_dvigs = tuple(sheet.col_slice(19, 2))   # колонка 19 - номера двигателей
    cells_acts = tuple(sheet.col_slice(12, 2))    # колонка 12 - номера актов рекламаций ПРИОБРЕТАТЕЛЯ изделия

    # вспомогательные списки двигателей и актов рекламаций
    dvigs = tuple(str(int(cell.value)) if type(cell.value) is float else cell.value for cell in cells_dvigs)
    ...
    for n in num1:           # перебираем входящий список двигателей и ищем двигатель во вспомогательном списке dvigs
        if n not in dvigs:   # если во вспомогательном списке нет - печатаем результат
            text_1.insert(1.0, 'Двигателя {0} в базе нет\n'.format(n))   # вывод текста

        for i, cell in enumerate(cells_dvigs):
            if n in str(cell.value):  # если в списке cells_dvigs двигатель есть - печатаем номер строки таблицы Excel
                row = i + 3
    ...
    for n in num2:           # перебираем входящий список актов и ищем акт во вспомогательном списке acts
        ...
        for i, cell in enumerate(cells_acts):
            if n in str(cell.value):  # если в списке cells_acts акт есть - печатаем номер строки таблицы Excel
                row = i + 3
...
def otchet_output():  # функция подготовки выборки из базы и печати отчета
    ...
    with open(r'//Server/otk/1 ГАРАНТИЯ на сервере/Отчет по результатам поиска по базе ОТК.txt', 'w',
        ...
        if num1:  # если есть список двигателей из строки ввода
            print('Номера двигателей:', ', '.join(str(n) for n in num1), file=res_file)  # печатаем номера двигателей
            print(file=res_file)
        ...
        for n in num1:  # ищем двигатель во вспомогательном списке dvigs
            ...
            for i, cell in enumerate(cells_dvigs):
                if n in str(cell.value):  # если в списке cells_dvigs двигатель есть - печатаем информацию из ячеек Excel
                    row = i + 3
                    ...
                    print(sheet.cell_value(row - 1, t.ind('Наименование изделия')), '|',
                          sheet.cell_value(row - 1, t.ind('Обозначение изделия')), '|',
                          'зав.№:', sheet.cell_value(row - 1, t.ind('Заводской номер изделия')),
                    ...
                    if sheet.cell_value(row - 1, t.ind('Номер рекламационного акта ПРИОБРЕТАТЕЛЯ изделия')):
                        print('Р/А №:',
                              sheet.cell_value(row - 1, t.ind('Номер рекламационного акта ПРИОБРЕТАТЕЛЯ изделия')),
                              '|', end=' ', file=res_file)
                    else:
                        print('Р/А №: акта нет', '|', end=' ', file=res_file)
                    print('Дефект:', sheet.cell_value(row - 1, t.ind('Заявленный дефект изделия')), file=res_file)
        ...
        if num2:   # если есть список актов из строки ввода
            print('Номер акта:', ', '.join(n for n in num2), file=res_file)  # печатаем номера актов рекламаций
            print(file=res_file)
        ...
        for n in num2:   # ищем акт во вспомогательном списке acts
            ...
            for i, cell in enumerate(cells_acts):
                if n in str(cell.value):   # если в списке cells_acts есть акт - печатаем информацию из ячеек Excel
                    row = i + 3
    ...
    messagebox.showinfo('ИНФОРМАЦИЯ', 'Отчет сохранен в папке "ГАРАНТИЯ на сервере"')


window = tk.Tk()

# Создается окно с заголовком
window.iconbitmap('//Server/otk/1 ГАРАНТИЯ на сервере/ЗНАЧКИ_Логотипы/IconBZA.ico')   # меняем логотип Tkinter (перышко) на свой логотип
window.title('ПОИСК ПО БАЗЕ РЕКЛАМАЦИЙ ОТК')     # название заголовка в окне приложения
...
lbl = tk.Label(master=frm_form, text='1. Введите ГОД в котором будет осуществляться поиск:   ', font=("Arial Bold", 10))
...
lbl_1 = tk.Label(master=frm_form_1, text='2. ПОИСК ОСУЩЕСТВЛЯЕТСЯ ПО:', font=("Arial Bold", 10))
lbl_null_1 = tk.Label(master=frm_form_1, text='')  # пустая строка
lbl_dvig_1 = tk.Label(master=frm_form_1, text='- НОМЕРУ ДВИГАТЕЛЯ.', font=("Arial Bold", 10))
...
lbl_act_1 = tk.Label(master=frm_form_1, text='- ПО НОМЕРУ АКТА РЕКЛАМАЦИИ.', font=("Arial Bold", 10))
...
text_1 = tk.Text(master=frm_form_2, width=85, height=7, background='white', font=("Arial Bold", 10))
...
bnt_3 = tk.Button(master=frm_buttons_2, text='СДЕЛАТЬ ОТЧЕТ', font=("Arial Bold", 10), command=otchet_output)
...
window.mainloop()
>>>>>>> 8654837cad4c23ca03c99e0366f3e549464f04f2
