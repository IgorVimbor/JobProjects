...
year_now = paths_home.year_now  # текущий год
date_end_new = paths_home.date_new  # дата составления отчета - сегодняшняя дата

# импортируем путь до файла базы данных номеров строк
database = paths_home.enquiry_period_database

# импортируем путь до файла базы рекламаций ОТК с учетом текущего года
file = paths_home.file_database

# импортируем путь до файла ТХТ в который будет записываться справка
res_file_txt = paths_home.enquiry_period_txt

# импортируем путь до файла Excel в который будет записываться справка
res_file_excel = paths_home.enquiry_period_excel
# ----------------------------------------------------------------------------------------------------------------------------
