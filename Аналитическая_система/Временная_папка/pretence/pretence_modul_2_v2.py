...
class ActsFromJournal:
    def __init__(self, sheet_name: str, new_acts: List[str]):
        ...
        self.file_path = f"{paths_home.journal_pretence}_{self.sheet_name.split()[0]}.xlsx"
    ...
    def print_results(self, write_func: Optional[Callable[[str], None]] = None) -> None:
        ...
        output("\nСписок актов: " + str(self.numbers_acts))
        output("Всего актов: " + str(len(self.numbers_acts)))

        for year, acts in self.years_list_acts.items():
            output(f"Акты {year} года: {acts}")
