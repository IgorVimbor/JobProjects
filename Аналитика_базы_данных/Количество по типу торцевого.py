...
df = pd.concat([data_2023, data_2022, data_2021, data_2020])        # (784, 7)

# удаляем строки датафрейма, в которых столбец 'Дата изготовления изделия' содержит пропуски
df.dropna(subset=['Дата изготовления изделия'],
          inplace=True)   # (712, 7) удалили 72 строки
...
df['Поставщик дефектного комплектующего'] = df['Поставщик дефектного комплектующего'].replace({'КАСО\nAAHU-15.9x36.45/42x\n10.8 SIC-SIC 07.05.19': 'KACO',
                                                                                               'КАСО': 'KACO',
...
df.loc[(df['Дата изготовления изделия'] < '2019-05-01')
       & df['Поставщик дефектного комплектующего'].isna(), 'Поставщик дефектного комплектующего'] = 'UMBRA'

df.loc[((df['Дата изготовления изделия'].isin(['2021-03-01', '2022-12-01'])) | (df['Дата изготовления изделия'] > '2022-02-01'))
       & df['Поставщик дефектного комплектующего'].isna(), 'Поставщик дефектного комплектующего'] = 'IIC'

df.loc[((df['Дата изготовления изделия'].between('2019-05-01', '2022-02-01')) | ~(df['Дата изготовления изделия'].isin(['2021-03-01', '2022-12-01'])))
       & df['Поставщик дефектного комплектующего'].isna(), 'Поставщик дефектного комплектующего'] = 'KACO'
# ------------------------------------------------------------------------------------------------------
...
new_df = df.groupby('Поставщик дефектного комплектующего').agg(count=('Поставщик дефектного комплектующего', pd.Series.count),
                                                               percent_part=('Поставщик дефектного комплектующего', lambda s: (
                                                                   s.value_counts() * 100 / all_count).round(2)),
                                                               probeg=('Пробег, наработка', lambda s: s.mean().round(2)))

print('\n\n', new_df)
#                                      count  percent_part    probeg
# Поставщик дефектного комплектующего
# IIC                                    184         24.60  44408.90
# KACO                                   418         55.88  72163.51
# UMBRA                                  146         19.52  84619.68
