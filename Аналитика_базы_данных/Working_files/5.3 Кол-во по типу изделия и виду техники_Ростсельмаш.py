...
for d in (df_2024, df_2023, df_2022):
    ...
    tmp_df = (
        d.groupby("Месяц регистрации")
        .agg(Total=("Обозначение изделия", "count"))
        .reset_index()
    )
    ...
    with open(
        "//Server/otk\ОТЧЕТНОСТЬ БЗА/АНАЛИЗ дефектности БЗА/Количество дефектов вн 536 на Ростсельмаш.txt",
        "a",
    ) as file:
