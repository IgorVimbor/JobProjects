...
with open(f"//Server/otk/Support_files_не_удалять!!!/НЕТ актов АСП - {date.today()}.txt", "w", encoding="utf-8") as f:
    print(f"\tПеречень актов рекламаций по которым НЕТ актов исследования на {date.today()}", file=f)
...
# print(df_gp_not_act)
# сохраняем в файл .txt
with open(f"//Server/otk/Support_files_не_удалять!!!/НЕТ актов ГП - {date.today()}.txt", "w", encoding="utf-8") as f:
    print(f"\n\n\tПеречень актов рекламаций по которым НЕТ актов исследования ГП на {date.today()}", file=f)
...
df["DIFF"] = (df["Дата акта исследования"] - df["Дата поступления изделия"]) / np.timedelta64(1, "D")
