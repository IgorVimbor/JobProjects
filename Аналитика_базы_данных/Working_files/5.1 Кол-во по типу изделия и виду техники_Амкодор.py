...
df_amkodor = df_2023[
    df_2023["Транспортное средство (установка)"].str.contains("Амкодор") == True
]

# результирующие датафреймы (обозначение изделия и количество в базе)
# за 2023 год
res_1 = (
    df_2023.groupby("Обозначение изделия")["Месяц регистрации"]
    .count()
...
# за 2022 год
res_2 = (
    df_2022.groupby("Обозначение изделия")["Месяц регистрации"]
    .count()
    .sort_values(ascending=False)
)
# print(res_2)
...
res = (
    df.groupby("Обозначение изделия")["Месяц регистрации"]
    .count()
    .sort_values(ascending=False)
)
# print(res)
...
with open(
    "//Server/otk/ПРОГРАММА КАЧЕСТВА и ЦЕЛИ по КАЧЕСТВУ/Программа качества и мероприятия на год/2024/График ММЗ.txt",
    "a",
) as file:
    print("\nДанные за 2023 год\n\n", res_1, file=file)
    print(file=file)
