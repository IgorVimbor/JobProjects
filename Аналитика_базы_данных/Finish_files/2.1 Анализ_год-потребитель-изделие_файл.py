...
name_cols = {1: 'Дата изготовления изделия',
             2: 'Пробег, наработка',
...
file_out = f'//Server/otk/ОТЧЕТНОСТЬ БЗА/АНАЛИЗ дефектности БЗА/Отчет за {year} по_{client}_' + str(
    date.today()) + '.txt'
...
all_frame = df[(df['Период выявления дефекта (отказа)'] == client)
               &
...
def get_lst_sort(iterable):
    ...
    for key in dct_count.keys():
        if type(key) == date:
            lst_sort_value = sorted(dct_count.items(), key=lambda x: (
                x[1], datetime.strptime(x[0], '%m.%y')), reverse=True)
        else:
            lst_sort_value = sorted(
                dct_count.items(), key=lambda x: x[1], reverse=True)
