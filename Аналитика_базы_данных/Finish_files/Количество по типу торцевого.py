...
new_df = df.groupby(
    'Поставщик дефектного комплектующего').agg(
        count=('Поставщик дефектного комплектующего', pd.Series.count),
        percent_part=(
            'Поставщик дефектного комплектующего', lambda s: (s.value_counts() * 100 / all_count).round(2)),
        probeg=(
            'Пробег, наработка', lambda s: s.mean().round(2))
)
