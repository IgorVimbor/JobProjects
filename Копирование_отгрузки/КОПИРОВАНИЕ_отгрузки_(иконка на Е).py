...
window.iconbitmap('E://ЗНАЧКИ_Логотипы/IconGreen.ico')             # меняем логотип Tkinter (перышко) на свой логотип
window.title('КОПИРОВАНИЕ ДАННЫХ ПО ОТГРУЗКЕ ИЗ ТАБЛИЦЫ ОСиМ')     # название заголовка в окне приложения
...
frame1 = tk.Frame(master=frm_form_1, relief=tk.SUNKEN, borderwidth=3, width=300, height=270, bg="white")
frame1.pack(fill=tk.BOTH, side=tk.LEFT)

# Создается рамка frame2
frame2 = tk.Frame(master=frm_form_1, relief=tk.SUNKEN, borderwidth=3, width=300, bg="white")
...
bnt_1 = tk.Button(master=frm_buttons_1, text='КОПИРОВАТЬ', font=("Arial Bold", 10), fg="green", command=progress_line)
bnt_2 = tk.Button(master=frm_buttons_1, text='СОХРАНИТЬ', font=("Arial Bold", 10), fg="green", command=get_save)
...
progress_bar = ttk.Progressbar(frm_form_3, orient="horizontal", mode="determinate", length=500, maximum=200, value=0)
...
bnt_2 = tk.Button(master=frm_buttons_2, text='ЗАКРЫТЬ ОКНО', font=("Arial Bold", 10), fg="red", command=close_window)
